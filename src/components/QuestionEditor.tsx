...
export const QuestionEditor: React.FC<QuestionEditorProps> = ({ onBack }) => {
  ...
  const [subjectAbbreviation, setSubjectAbbreviation] = useState<string[]>([]);
  const [levelAbbreviation, setLevelAbbreviation] = useState("");
  ...
  const handleSubjectArea = ((value: string) => {
    let temp = [];
    if (value === '') {temp.push('All') || temp.push('All') || temp.push('All')};
    if (value === 'SS') {temp.push('SS'); temp.push("Social Studies"); temp.push('Social');}
    if (value === 'SC') {temp.push("SC"); temp.push('Science'); temp.push("SC")}
    if (value === 'LA') {temp.push("LA"); temp.push("Language Arts"); temp.push("English")}
    if (value === 'MA') {temp.push("MA"); temp.push('Math'); temp.push('Mathmatics')}
    if (value === 'AH') {temp.push("AH"); temp.push("ARTS & HUMANITIES"); temp.push('Arts')}
    setSubjectAbbreviation([...temp]);
    setFilters({...filters, subjectArea: value})
  });
  const handleLevel = ((value: string) => {
    if (value === "") setLevelAbbreviation('all');
    if (value === "EL") setLevelAbbreviation("1");
    if (value === "MS") setLevelAbbreviation("2");
    if (value === "HS") setLevelAbbreviation("3");

    setFilters({...filters, level: value as '' | 'EL' | 'MS' | 'HS' })
  })

  const applyFilters = useCallback(() => {
    let filtered = [...questions];
    if (filters.subjectArea.length > 0) {
      filtered = filtered.filter((q) => q.subjectArea === filters.subjectArea || q.subjectArea === subjectAbbreviation[0] || q.subjectArea === subjectAbbreviation[1] || q.subjectArea === subjectAbbreviation[2]);
      
    }
    if (filters.level) {
      filtered = filtered.filter((q) => q.level === filters.level || q.level === levelAbbreviation);
    }
    if (filters.minYear) {
  ...
  if (previewingQuestions) {
    ...
    setTemp(paginatedQuestions.map((q) => q.questionText));
    return (
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center px-4 pt-[35rem] overflow-auto">
              ...
              <select
                ...
                onChange={
                  (e) => {
                    ...
                    handleSubjectArea(e.target.value);
                  }
                }
                className="w-full bg-purple-900 text-white p-2 rounded border border-purple-500/30"
              >
        ...
        {/* Selection controls and question list */}
        {/* Bulk select, individual select, and bulk delete controls */}
        {filteredQuestions.length > 0 && (
              ...
              <label className="flex items-center gap-2 text-white font-bold">
                <input
                  ...
                  checked={
                    ...
                    filteredQuestions
                      .slice((currentPage - 1) * itemsPerPage, filteredCount).length > 0
                  }
                  onChange={() => {
                    // Get visible IDs
                    const allVisibleQuestionIds = filteredQuestions
                      .slice((currentPage - 1) * itemsPerPage, filteredCount)
                      .map(q => q.id);
                    const isAllSelected =
                      allVisibleQuestionIds.length > 0 &&
                      allVisibleQuestionIds.every(id => selectedQuestions.includes(id));
                    if (isAllSelected) {
                      setSelectedQuestions(
                        selectedQuestions.filter(id => !allVisibleQuestionIds.includes(id))
                      );
                    } else {
                      setSelectedQuestions([
                        ...selectedQuestions,
                        ...allVisibleQuestionIds.filter(id => !selectedQuestions.includes(id)),
                      ]);
                    }
                  }}
              ...
              <button
                onClick={async () => {
                  if (
                    ...
                    !window.confirm(
                      `Delete ${selectedQuestions.length} selected question(s)?`
                    )
                  )
                    return;
                  try {
                ...
                className="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded font-bold disabled:opacity-50"
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto">
          {filteredQuestions
            ...
            <div
              key={q.id}
              className="relative bg-purple-950 rounded-xl p-6 border-2 border-purple-800"
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex gap-3 items-center">
                  {/* rest of card contents ... */}
                  <span className="bg-purple-600 text-white px-3 py-1 rounded font-bold text-sm">
                    {q.subjectArea === "SS" || q.subjectArea === "Social Studies"
                      ? "Social Studies"
                      : q.subjectArea === "SC" || q.subjectArea === "Science"
                      ? "Science"
                      : q.subjectArea === "LA" ||
                        q.subjectArea === "Language Arts"
                      ? "Language Arts"
                      : q.subjectArea === "MA" || q.subjectArea === "Math"
                      ? "Math"
                      : "Arts"}
                  </span>
                  <span className="bg-cyan-600 text-white px-3 py-1 rounded font-bold text-sm">
                  ...
                  {q.validationStatus && (
                    ...
                    >
                      {q.validationStatus.charAt(0).toUpperCase() +
                        q.validationStatus.slice(1)}
                    </span>
                  )}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="bg-red-600 hover:bg-red-500 text-white px-4 py-1 rounded font-bold text-sm mr-3"
                  >
                    DELETE
                  </button>
                ...
                onChange={() => {
                  setSelectedQuestions(
                    selectedQuestions.includes(q.id)
                      ? selectedQuestions.filter(qid => qid !== q.id)
                      : [...selectedQuestions, q.id]
                  );
                }}
                className="absolute right-1 ml-[0.75rem] mt-1 w-5 h-5 accent-yellow-400 bg-white border border-purple-500 rounded shadow z-10"
              />
                </div>
              </div>
